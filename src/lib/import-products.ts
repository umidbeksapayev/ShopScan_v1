...
import { normalizeBarcode, computeLowStockThreshold } from "@/lib/utils";

/**
 * Ommaviy import — sof (framework'siz) yadro: CSV parsing, ustun aniqlash,
 * raqam/tur parsing, qator validatsiyasi va dedupe. Hammasi unit-test qilinadi.
 * Excel (.xlsx) o'qish brauzer-bog'liq (exceljs) — u `lib/excel.ts` da.
 */

// ============================================================
// Ustun nomlari (3 til + sinonimlar). Aniq (normalizatsiyalangan) moslik.
// ============================================================
...
export function parseNumber(raw: unknown): number | null {
  ...
  if (hasComma && hasDot) {
    // Oxirgi ajratuvchi = kasr ajratuvchisi
    ...
    s = s.replace(/,/g, "."); // vergul = kasr ajratuvchisi
  }
  s = s.replace(/[^0-9.\-]/g, "");
...
export function parseCsv(text: string): string[][] {
  ...
  // Butunlay bo'sh qatorlarni olib tashlash
...
export interface ImportPreviewRow {
  ...
  errors: string[]; // i18n kodlari (import.err.<code>)
  saleType: SaleType;
  costPrice: number;
  sellingPrice: number;
  quantity: number;
...
export interface ImportPreviewResult {
  headerError: boolean; // majburiy ustunlar topilmadi
  columns: ColumnMap;
...
export function buildPreview(
  ...
  // Majburiy ustunlar: name, cost, selling, quantity (type ixtiyoriy → default dona)
  const headerError =
  ...
  const seen = new Set<string>(); // shu fayl ichidagi barcode'lar
